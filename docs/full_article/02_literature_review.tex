...
\citet{pellicer2024explainable} present a particularly relevant approach to vegetation forecasting that prioritizes interpretability alongside accuracy. Their work introduces a growth curve decoder that generates predictions through explicit modeling of vegetation growth dynamics using saturation growth functions.

The key insight of \citet{pellicer2024explainable} is that vegetation temporal dynamics can be effectively approximated by parametric growth curves of the form:
\begin{equation}
    \delta(t) = A \cdot (1 - e^{-\lambda t}) + B
\end{equation}
where $A$ represents growth amplitude, $\lambda$ the growth rate, and $B$ a baseline offset. These parameters have direct biophysical interpretations: amplitude captures the magnitude of vegetation response, rate determines how quickly the response develops, and offset accounts for systematic biases.

Their architecture uses this growth curve formulation to predict next-timestep vegetation states, with the neural network learning to estimate growth parameters conditioned on historical observations and meteorological context. This approach provides inherent interpretability---predictions can be explained in terms of predicted growth dynamics rather than opaque network activations.

Importantly, \citet{pellicer2024explainable} also demonstrate improved performance under extreme weather events (droughts, heatwaves), suggesting that explicit modeling of growth dynamics enables more robust extrapolation beyond training data distributions.
...
Existing forecasting methods, including the approach of \citet{pellicer2024explainable}, typically predict vegetation states iteratively, generating next-timestep predictions that are then fed back as input for subsequent predictions. This autoregressive approach suffers from error accumulation over long horizons and requires multiple forward passes through the network.
